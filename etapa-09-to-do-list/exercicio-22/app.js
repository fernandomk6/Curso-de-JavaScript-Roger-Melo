...
const sortedByIdCharacters = [ ...characters ]
  .sort((character1, character2) => character1.id - character2.id)

// console.log(sortedByIdCharacters)
...
const firstNumberbiggerThan50 = randomNumbers.find(number => number > 50)

// console.log(firstNumberbiggerThan50)
...
const sortedPeopleInverted = [ ...people ]
  .sort()
  .reverse()

// console.log(sortedPeopleInverted)
...
const ingredientsText = ingredients
  .map(ingredient => `${ingredient} ${ingredient[ingredient.length - 1] === 'a' ? 'cozida' : 'cozido'}`)
  .join(', ')
...
const totalDisneyViewer = topBrazilmovies
  .reduce((accumulator, { distributedBy, peopleAmount }) => 
    distributedBy === 'Disney' ? accumulator + peopleAmount : accumulator, 0)

// console.log(totalDisneyViewer)
...
const dogs = pets
  .filter(pet => pet.type === dogType)
  .map(dog => {
    const humanAge = dog.age * 7

    return { ...dog, age: humanAge }
  })

...
// console.log(myAge) // Erro

// A variável "myAge" não existe no escopo onde foi chamada.
// Ela existe no escopo da função logMyAge, logo ela so pode ser usada
// dentro desse escopo. Não pode ser acessada fora dele.
...
const car = {
  ...
  getColorMessage () {
    ...
    const colorsMessage = colors.join(', ')
      .replace(`, ${lastColor}`, ` e ${lastColor}`)

    return `O ${this.name} está disponível nas cores ${colorsMessage}.`
  }
}
...
const lancerColors = car.getColorMessage()

console.log(lancerColors)
...
const carInfo = `O carro é um ${car['brand']} ${car['name']}`
